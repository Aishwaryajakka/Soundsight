...
export default function SettingsScreen() {
  ...
  const clearTranscripts = () => Alert.alert('Clear transcripts?', 'This removes all locally stored live-caption text.', [{ text: 'Cancel', style: 'cancel' }, { text: 'Clear Transcripts', style: 'destructive', onPress: state.clearTranscripts }]);
  ...
  return <SafeAreaView className="flex-1 overflow-hidden bg-[#021E32]" edges={['top', 'left', 'right']}>
    ...
    <ScrollView className="z-10 flex-1" contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 32 }} showsVerticalScrollIndicator={false}>
      ...
      <View className="rounded-2xl border border-[#55C2E8]/15 bg-[#062C45] px-4">
        ...
        <Pressable accessibilityRole="button" onPress={state.testHaptic} className="h-12 items-center justify-center border-t border-[#55C2E8]/10"><Text className="text-sm font-semibold text-[#55C2E8]">Test Haptic</Text></Pressable>
      </View>

...
export default function SettingsScreen() {
  ...
  const setSensitivity = (value: 'Low' | 'Medium' | 'High') => {
    state.setDetectionSensitivity(value);
    state.showFeedback(state.liveConnectionState === 'connected'
      ? `${value} sensitivity saved.`
      : `${value} sensitivity saved. It will apply when live AI reconnects.`);
  };
  const setFadeDuration = (value: '5s' | '10s' | '20s') => {
    state.setKeepEventsVisibleDuration(value);
    state.showFeedback(`Map fade duration set to ${value}.`);
  };
  const toggleConfidence = () => {
    const enabled = !state.showConfidence;
    state.setShowConfidence(enabled);
    state.showFeedback(`Confidence percentages ${enabled ? 'shown' : 'hidden'}.`);
  };
  const toggleContours = () => {
    const enabled = !state.showSoundIntensity;
    state.setShowSoundIntensity(enabled);
    state.showFeedback(`Luminous contour waves ${enabled ? 'enabled' : 'disabled'}.`);
  };
  const toggleHaptics = () => {
    const enabled = !state.hapticAlertsEnabled;
    state.setHapticAlertsEnabled(enabled);
    state.showFeedback(`Haptic feedback ${enabled ? 'enabled' : 'disabled'}.`);
  };
  ...
  return <SafeAreaView className="flex-1 overflow-hidden bg-[#021E32]" edges={['top', 'left', 'right']}>
    ...
    <ScrollView className="z-10 flex-1" contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 32 }} showsVerticalScrollIndicator={false}>
      ...
      <View className="rounded-2xl border border-[#55C2E8]/15 bg-[#062C45] p-4">
        ...
        <View className="mt-3 h-11 flex-row rounded-xl border border-[#55C2E8]/20 bg-[#021E32] p-1">{(['Low', 'Medium', 'High'] as const).map((value) => <Pressable accessibilityRole="button" accessibilityState={{ selected: state.detectionSensitivity === value }} accessibilityLabel={`${value} microphone sensitivity`} key={value} onPress={() => setSensitivity(value)} className={`flex-1 items-center justify-center rounded-lg ${state.detectionSensitivity === value ? 'bg-[#55C2E8]' : ''}`}><Text className={`text-sm font-medium ${state.detectionSensitivity === value ? 'text-[#021E32]' : 'text-[#C6E8F5]'}`}>{value}</Text></Pressable>)}</View>
        ...
        <View className="mt-3 h-11 flex-row rounded-xl border border-[#55C2E8]/20 bg-[#021E32] p-1">{(['5s', '10s', '20s'] as const).map((value) => <Pressable accessibilityRole="button" accessibilityState={{ selected: state.keepEventsVisibleDuration === value }} accessibilityLabel={`${value} map fade duration`} key={value} onPress={() => setFadeDuration(value)} className={`flex-1 items-center justify-center rounded-lg ${state.keepEventsVisibleDuration === value ? 'bg-[#55C2E8]' : ''}`}><Text className={`text-sm font-medium ${state.keepEventsVisibleDuration === value ? 'text-[#021E32]' : 'text-[#C6E8F5]'}`}>{value}</Text></Pressable>)}</View>
      </View>
      <SectionTitle>Accessibility &amp; Display</SectionTitle>
      <View className="rounded-2xl border border-[#55C2E8]/15 bg-[#062C45] px-4">
        {[
          { label: 'Show Confidence Percentages', description: 'Show AI confidence for detected sounds.', enabled: state.showConfidence, toggle: toggleConfidence },
          { label: 'Luminous Sound Contour Waves', description: 'Show visual sound-wave contours on the map.', enabled: state.showSoundIntensity, toggle: toggleContours },
          { label: 'Haptic Feedback', description: 'Vibrate for important detected sounds.', enabled: state.hapticAlertsEnabled, toggle: toggleHaptics },
      ...
      <View className="rounded-2xl border border-[#55C2E8]/15 bg-[#062C45] p-4"><Text className="text-[15px] font-semibold text-[#F7FBFD]">Sound History</Text><Text className="mt-1 text-xs leading-5 text-[#A9C6D8]">Detected sound events are stored locally on this device.</Text><Pressable accessibilityRole="button" onPress={clearSoundHistory} className="mt-3 h-10 items-center justify-center rounded-xl border border-[#55C2E8]/30"><Text className="text-sm font-semibold text-[#55C2E8]">Clear Sound History</Text></Pressable><View className="my-4 h-px bg-[#55C2E8]/10" /><View className="flex-row items-center justify-between"><Text className="text-[15px] font-semibold text-[#F7FBFD]">Conversation Transcripts</Text><Text className="text-xs font-semibold text-[#55C2E8]">7 days</Text></View><Text className="mt-1 text-xs leading-5 text-[#A9C6D8]">Live caption text is stored locally and automatically deleted after 7 days.</Text><Pressable accessibilityRole="button" onPress={clearTranscripts} className="mt-3 h-10 items-center justify-center rounded-xl border border-[#55C2E8]/30"><Text className="text-sm font-semibold text-[#55C2E8]">Clear Transcripts</Text></Pressable></View>

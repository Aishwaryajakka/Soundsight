...
export function deriveHistoryAnalytics(events: SoundEvent[], now = new Date()): HistoryAnalytics {
  ...
  const breakdown = [...counts.entries()].sort((left, right) => right[1] - left[1] || left[0].localeCompare(right[0])).slice(0, 4).map(([label, count]) => ({ label, count }));
